      ...
      await db.runTransaction(async (t) => {
        ...
        if (status === 'SUCCESS') {
          ...
          if (payment?.paymentType === 'SUBSCRIPTION' && payment?.referenceId) {
            subRef = db.collection('subscriptions').doc(payment.referenceId);
            subDoc = await t.get(subRef);
          }
          ...
          if (subDoc && subDoc.exists) {
            ...
            if (payment.metadata?.packageId) {
              updateData.packageId = payment.metadata.packageId;
            }

            t.update(subRef, updateData);
