...
export default function AdminPaymentHistory() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">Payment History</h1>
        <p className="mt-2 text-slate-600">Master ledger of all transactions across the platform.</p>
      </div>

      {error && <div className="text-red-500 font-medium p-4 bg-red-50 rounded-xl border border-red-100">{error}</div>}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
              ...
              {loading ? (
                ...
                payments.map((payment) => (
                  <tr key={payment.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-indigo-600">{payment.id}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-slate-900">{payment.tenantName}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600">{payment.packageName}</td>
        ...
        <div className="px-6 py-4 border-t border-slate-200 bg-slate-50 flex items-center justify-between">
          <button
            ...
            className="px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            Previous
          </button>
